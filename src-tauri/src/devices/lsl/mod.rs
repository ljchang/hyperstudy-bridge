...
pub use resolver::{StreamResolver, DiscoveredStream, StreamFilter};
pub use inlet::{InletManager, InletConfig};
pub use outlet::{OutletManager, OutletConfig};
...
impl LslDevice {
    ...
    pub fn with_performance_monitoring(
        ...
        device.set_performance_callback(move |device_id, latency, bytes_sent, bytes_received| {
            ...
            tokio::spawn(async move {
                monitor.record_device_operation(&device_id, latency, bytes_sent, bytes_received).await;
            });
        });
    ...
    async fn create_bridge_outlets(&self) -> Result<(), DeviceError> {
        ...
        for device_type in &device_types {
            let outlet_id = self.outlet_manager
            ...
            // Start the outlet
            self.outlet_manager.start_outlet(&outlet_id)
                .await
                .map_err(|e| DeviceError::ConfigurationError(e.to_string()))?;
    ...
    pub async fn discover_streams(&self, filters: Vec<StreamFilter>) -> Result<Vec<DiscoveredStream>, LslError> {
    ...
    pub async fn send_bridge_data(&self, device_type: &str, data: Vec<u8>, timestamp: Option<f64>) -> Result<(), DeviceError> {
        let data_len = data.len() as u64; // Capture length before moving data

        let outlets = self.bridge_outlets.read().await;
        let outlet_id = outlets.get(device_type)
            .ok_or_else(|| DeviceError::ConfigurationError(format!("No outlet for device type: {}", device_type)))?;

        // Convert data to appropriate sample format
        let sample_data = match device_type {
            "ttl" => {
                // TTL data should be string markers
                let marker = String::from_utf8(data)
            ...
            "kernel" | "pupil" | "biopac" => {
                ...
                if data.len() % 4 != 0 {
                    return Err(DeviceError::InvalidData("Data length not multiple of 4".to_string()));
                }
            ...
            _ => return Err(DeviceError::InvalidData(format!("Unknown device type: {}", device_type))),
        };
        ...
        self.outlet_manager.send_sample(outlet_id, sample)
            .await
    ...
    async fn process_command(&self, command: LslCommand) -> LslResponse {
        match command {
            LslCommand::DiscoverStreams { filters, timeout: _ } => {
            ...
            LslCommand::CreateOutlet { device_type, device_id, config } => {
                match self.outlet_manager.create_device_outlet(&device_id, &device_type, config).await {
            ...
            LslCommand::SendSample { outlet_id, data, timestamp } => {
            ...
            LslCommand::SyncTime => {
                match self.time_sync.synchronize().await {
                    Ok(()) => LslResponse {
                        success: true,
                        data: serde_json::json!({}),
                        error: None,
    ...
    async fn start_command_processing(&mut self) -> Result<(), DeviceError> {
        let mut receiver = self.command_receiver.take()
            .ok_or_else(|| DeviceError::ConfigurationError("Command receiver not available".to_string()))?;
...
impl Device for LslDevice {
    async fn connect(&mut self) -> Result<(), DeviceError> {
        ...
        info!("LSL device connected successfully in {:?}", connection_latency);
    ...
    async fn disconnect(&mut self) -> Result<(), DeviceError> {
        ...
        self.outlet_manager.stop_all()
            .await
            .map_err(|e| DeviceError::CommunicationError(e.to_string()))?;

        self.inlet_manager.stop_all()
            .await
            .map_err(|e| DeviceError::CommunicationError(e.to_string()))?;
    ...
    async fn receive(&mut self) -> Result<Vec<u8>, DeviceError> {
        ...
        if let Some(inlet_id) = inlet_ids.first() {
            match self.inlet_manager.pull_sample(inlet_id, Duration::from_millis(100)).await {
                Ok(Some(sample)) => {
