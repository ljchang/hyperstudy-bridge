use super::{Device, DeviceConfig, DeviceError, DeviceInfo, DeviceStatus, DeviceType};
use async_trait::async_trait;
use tokio::net::TcpStream;
use tokio::io::{AsyncReadExt, AsyncWriteExt};
use tracing::{error, info, debug};

const DEFAULT_PORT: u16 = 5000;
const START_ACQUISITION: &str = "START";
const STOP_ACQUISITION: &str = "STOP";
const SET_MARKER: &str = "MARKER";

#[derive(Debug, Clone)]
pub struct ChannelConfig {
    pub channel_id: u32,
    pub name: String,
    pub enabled: bool,
    pub sampling_rate: u32,
}

#[derive(Debug)]
pub struct BiopacDevice {
    socket: Option<TcpStream>,
    server_address: String,
    port: u16,
    status: DeviceStatus,
    config: DeviceConfig,
    channels: Vec<ChannelConfig>,
    buffer: Vec<u8>,
    acquiring: bool,
}

impl BiopacDevice {
    pub fn new(server_address: String) -> Self {
        Self {
            socket: None,
            server_address,
            port: DEFAULT_PORT,
            status: DeviceStatus::Disconnected,
            config: DeviceConfig::default(),
            channels: Vec::new(),
            buffer: Vec::with_capacity(8192),
            acquiring: false,
        }
    }

    pub async fn start_acquisition(&mut self) -> Result<(), DeviceError> {
        if self.status != DeviceStatus::Connected {
            return Err(DeviceError::NotConnected);
        }

        self.send(START_ACQUISITION.as_bytes()).await?;
        self.acquiring = true;
        info!("Started Biopac data acquisition");
        Ok(())
    }

    pub async fn stop_acquisition(&mut self) -> Result<(), DeviceError> {
        if self.status != DeviceStatus::Connected {
            return Err(DeviceError::NotConnected);
        }

        self.send(STOP_ACQUISITION.as_bytes()).await?;
        self.acquiring = false;
        info!("Stopped Biopac data acquisition");
        Ok(())
    }

    pub async fn set_marker(&mut self, marker: &str) -> Result<(), DeviceError> {
        if self.status != DeviceStatus::Connected {
            return Err(DeviceError::NotConnected);
        }

        let command = format!("{} {}", SET_MARKER, marker);
        self.send(command.as_bytes()).await?;
        debug!("Set marker: {}", marker);
        Ok(())
    }
}

#[async_trait]
impl Device for BiopacDevice {
    async fn connect(&mut self) -> Result<(), DeviceError> {
        info!("Connecting to Biopac at {}:{}", self.server_address, self.port);
        self.status = DeviceStatus::Connecting;

        let addr = format!("{}:{}", self.server_address, self.port);

        match TcpStream::connect(&addr).await {
            Ok(socket) => {
                self.socket = Some(socket);
                self.status = DeviceStatus::Connected;
                info!("Successfully connected to Biopac");
                Ok(())
            }
            Err(e) => {
                self.status = DeviceStatus::Error;
                error!("Failed to connect to Biopac: {}", e);
                Err(DeviceError::ConnectionFailed(e.to_string()))
            }
        }
    }

    async fn disconnect(&mut self) -> Result<(), DeviceError> {
        info!("Disconnecting from Biopac");

        if self.acquiring {
            let _ = self.stop_acquisition().await;
        }

        if let Some(mut socket) = self.socket.take() {
            socket.shutdown().await
                .map_err(|e| DeviceError::CommunicationError(e.to_string()))?;
        }

        self.status = DeviceStatus::Disconnected;
        self.buffer.clear();
        Ok(())
    }

    async fn send(&mut self, data: &[u8]) -> Result<(), DeviceError> {
        if let Some(ref mut socket) = self.socket {
            socket.write_all(data).await
                .map_err(|e| DeviceError::CommunicationError(e.to_string()))?;

            socket.write_all(b"\n").await
                .map_err(|e| DeviceError::CommunicationError(e.to_string()))?;

            socket.flush().await
                .map_err(|e| DeviceError::CommunicationError(e.to_string()))?;

            Ok(())
        } else {
            Err(DeviceError::NotConnected)
        }
    }

    async fn receive(&mut self) -> Result<Vec<u8>, DeviceError> {
        if let Some(ref mut socket) = self.socket {
            self.buffer.clear();
            self.buffer.resize(8192, 0);

            match socket.read(&mut self.buffer).await {
                Ok(0) => {
                    self.status = DeviceStatus::Disconnected;
                    self.socket = None;
                    Err(DeviceError::ConnectionFailed("Connection closed by remote".to_string()))
                }
                Ok(n) => {
                    self.buffer.truncate(n);
                    Ok(self.buffer.clone())
                }
                Err(e) => Err(DeviceError::CommunicationError(e.to_string()))
            }
        } else {
            Err(DeviceError::NotConnected)
        }
    }

    fn get_info(&self) -> DeviceInfo {
        DeviceInfo {
            id: format!("biopac_{}_{}", self.server_address.replace('.', "_"), self.port),
            name: format!("Biopac MP150/160 ({}:{})", self.server_address, self.port),
            device_type: DeviceType::Biopac,
            status: self.status,
            metadata: serde_json::json!({
                "server_address": self.server_address,
                "port": self.port,
                "channels": self.channels.len(),
                "acquiring": self.acquiring,
            }),
        }
    }

    fn get_status(&self) -> DeviceStatus {
        self.status
    }

    fn configure(&mut self, config: DeviceConfig) -> Result<(), DeviceError> {
        self.config = config;

        if let Some(custom) = self.config.custom_settings.as_object() {
            if let Some(addr) = custom.get("server_address").and_then(|v| v.as_str()) {
                self.server_address = addr.to_string();
            }

            if let Some(port) = custom.get("port").and_then(|v| v.as_u64()) {
                self.port = port as u16;
            }

            if let Some(channels) = custom.get("channels").and_then(|v| v.as_array()) {
                self.channels.clear();
                for ch in channels {
                    if let Some(ch_obj) = ch.as_object() {
                        let channel = ChannelConfig {
                            channel_id: ch_obj.get("id").and_then(|v| v.as_u64()).unwrap_or(0) as u32,
                            name: ch_obj.get("name").and_then(|v| v.as_str()).unwrap_or("Unknown").to_string(),
                            enabled: ch_obj.get("enabled").and_then(|v| v.as_bool()).unwrap_or(true),
                            sampling_rate: ch_obj.get("sampling_rate").and_then(|v| v.as_u64()).unwrap_or(1000) as u32,
                        };
                        self.channels.push(channel);
                    }
                }
            }
        }

        Ok(())
    }

    async fn heartbeat(&mut self) -> Result<(), DeviceError> {
        if self.socket.is_some() {
            Ok(())
        } else {
            Err(DeviceError::NotConnected)
        }
    }
}